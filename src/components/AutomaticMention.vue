<template>
  <section>
    ...
    <el-card class="box-card">
      ...
      <div class="text item">
        商户编号 : {{AutomaticMentionObj.CStationid?AutomaticMentionObj.CStationid:"待查询"}}
      </div>
      <div class="text item">
        商户名称 : {{AutomaticMentionObj.CStationName?AutomaticMentionObj.CStationName:"待查询"}}
      </div>
      <div class="text item">
        总金额 : {{AutomaticMentionObj.CApplayPrice}}元
      </div>
      <div class="text item">
        可提现金额 :
        <span style="color:green; font-weight: bold;">{{AutomaticMentionObj.CBackPrice}}</span>元
      </div>
      <div class="text item">
        不可提现金额 :
        <span style="color:red; font-weight: bold;">{{AutomaticMentionObj.CFree}}</span>元
      </div>
    </el-card>
    <el-card class="box-card">
      <div slot="header" class="clearfix">
    ...
    <el-card class="box-card">
      ...
      <div class="text item">
        商户名称 : {{AutomaticMentionObj.CStationName?AutomaticMentionObj.CStationName:""}}
      </div>
      <div class="text item">
        截止时间 : <span style="color:#00c; font-weight: bold;">{{new Date() | getNewTimeDate}}</span>
      </div>
      <div class="text item">
        提现金额 : <span style="color:green; font-weight: bold;">{{AutomaticMentionObj.CApplayPrice}}</span>元
      </div>
      <div class="text item">
        手续费率 : {{AutomaticMentionObj.CFreeRate}}
      </div>
      <div class="text item">
        到账金额 : <span style="color:green; font-weight: bold;">{{AutomaticMentionObj.CBackPrice}}</span>元
      </div>
      <div class="text item">
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      application(){
        this.isSubmit = true;
        this.$store.dispatch('applicationSubmitBtn',{
          StationID:this.AutomaticMentionObj.CStationid,
          Password:this.loginPassword,
          FromDate:this.userSearch.FromDate
          ,ToDate:this.userSearch.ToDate
        })
          .then((balance)=>{
            this.$store.dispatch('SendMessage',{
              StationID:this.AutomaticMentionObj.CStationid,
              Password:this.loginPassword,
              TotalMoney:Number(balance)
            })
              .then((resulte)=>{
                this.$message({
                  message: '短信'+resulte,
